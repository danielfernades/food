...
load data local infile 'NUT_DATA.txt' into table nutrient_data
    ...
    set 
        str_error = if(@str_error='',null,@str_error),
        src_cd = if(@src_cd='',null,@src_cd),
        deriv_cd = if(@deriv_cd='',null,@deriv_cd),
        ref_ndb_no= if(@ref_ndb_no='',null,@ref_ndb_no),
        add_nutr_mark = if(@add_nutr_mark='',null,@add_nutr_mark),
        num_studies = if(@num_studies='',null,@num_studies),
        `min` = if(@min='',null,@min),
        `max` = if(@max='',null,@max),
        df = if(@df = '',null,@df),
        low_eb = if(@low_eb='',null,@low_eb),
        up_eb = if(@up_eb='',null,@up_eb),
        stat_cmt =if(@stat_cmt='',null,@stat_cmt),
        addmod_date =if(@addmod_date='',null,
            concat_ws('-',right(@addmod_date,4),left(@addmod_date,2),'01')),
        cc = null;
...
load data local infile 'WEIGHT.txt' into table weight 
    ...
    set 
        num_data_pts = if(@num_data_pts = '', null, @num_data_pts), 
        std_dev = if(@std_dev='', null, @std_dev);
...
load data local infile 'FOOTNOTE.txt' into table footnote 
    ...
    set nutr_no = if(@nutr_no='',null,@nutr_no);
...
(
    datasrc_id CHAR(5) NOT NULL comment 'Unique number identifying the reference/source.',
    authors VARCHAR(110) NOT NULL comment 'List of authors for a journal article or name of sponsoring organization for other documents.',
...
load data local infile 'DATA_SRC.txt' into table data_sources 
    ...
    set 
        ...
        vol_city=if(@vol_city='',null,@vol_city), 
        issue_state=if(@issue_state='',null,@issue_state), 
        start_page=if(@start_page='',null,@start_page), 
        end_page=if(@end_page='',null,@end_page);
...
load data local infile 'FOOD_DES.txt' into table food_desc 
    ...
    set 
        refuse = if(@refuse='',null,@refuse), 
        n_factor = if(@n_factor='',null,@n_factor), 
        pro_factor = if(@pro_factor='',null,@pro_factor), 
        fat_factor = if(@fat_factor='',null,@fat_factor), 
        cho_factor = if(@cho_factor='',null,@cho_factor);
